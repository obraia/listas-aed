...
public class TabelaHashProdutoAberto {

    private int M;	/// tamanho da tabela hash.
    /// M deve ser um número primo grande para diminuirmos a probabilidade de colisões.

    private Produto[] tabelaProdutos; 	/// tabela que referenciará todas as listas lineares encadeadas de produtos.
    ...
    public TabelaHashProdutoAberto(int tamanho) {
        this.M = tamanho;
        Produto[] auxTabela = new Produto[this.M];

        for (int i = 0; i < this.M; i++) {
            auxTabela[i] = new Produto(0, "", 0.0);
        }
    ...
    public int inserir(Produto produtoNovo) {
        ...
        if (this.pesquisar(codigoBarras).getNome() != "") {
            return -1;
        }

        int posicao = funcaoHash(codigoBarras);

        if (tabelaProdutos[posicao].getNome() != "") {
            return -1;
        }

        tabelaProdutos[posicao] = produtoNovo;
    ...
    public Produto pesquisar(int codigoBarras) {
        int posicao = funcaoHash(codigoBarras);
        return this.tabelaProdutos[posicao];
    }
    ...
    public void imprimir() {
        for (int i = 0; i < this.M; i++) {
            ...
            Produto aux = tabelaProdutos[i];
            if (aux.getNome() != "") {
                System.out.println("Código de barras: " + aux.getCodigoBarras() + ", Nome: " + aux.getNome() + ", Preço unitário: " + aux.getPrecoUnitario());
            } else {
